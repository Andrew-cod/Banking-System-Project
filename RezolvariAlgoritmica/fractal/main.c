...
{
  fprintf(f,"M %.1f %.1f ", x, y);	// move to origin of figure
  fprintf(f,"h %.1f ", side);	// %.1f : real with 1 digit after .
  fprintf(f,"v %.1f ", side);
  fprintf(f,"h %.1f ", -side);
  fprintf(f,"v %.1f ", -side);
}
...
int main(void)
{
  ...
  f=fopen("C:\\Users\\arapu\\Desktop\\svg.html","w");
